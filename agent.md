...
### 메모리 자동 업데이트 제안
다음과 같은 **중요한 변경**이 있을 때 **반드시** 사용자에게 메모리 업데이트 여부를 물어보세요:

**중요한 변경 기준**:
- ✅ 새로운 API 엔드포인트 추가/수정/삭제
- ✅ 새로운 기능 추가 (UI 페이지, 컴포넌트, 서비스 등)
- ✅ 아키텍처 패턴 변경 (Repository 추가, Service 구조 변경 등)
- ✅ 데이터 흐름 변경 (새로운 Hook, 캐시 전략 변경 등)
- ✅ 중요한 버그 수정 (로직 변경이 있는 경우)

**제안 방법**:
1. 작업 완료 후 "이 변경사항을 세레나 메모리에 반영하시겠습니까?" 물어보기
2. 업데이트 대상 메모리 파일명 명시 (예: `project_architecture_map`)
3. 어떤 내용을 추가/수정할지 간략히 설명

**제안하지 않아도 되는 경우**:
- ❌ 단순 문구 수정, 스타일 변경
- ❌ 로직 변경 없는 코드 정리 (포맷팅, 변수명 변경 등)
- ❌ 문서 파일만 수정한 경우
