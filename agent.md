...
### 문서 저장 위치
- **모든 프로젝트 문서는 최상단의 `docs/` 폴더에 저장**
- 기능별로 명확한 파일명 사용 (예: `authentication-flow.md`, `api-endpoints.md`)
- Markdown 형식 사용 (`.md`)

### 문서 작성 시기
다음과 같은 경우 문서를 작성하세요:
1. 새로운 주요 기능 추가 시 (예: 인증 시스템, 결제 시스템)
2. 복잡한 내부 동작 플로우가 있는 경우
3. API 엔드포인트 변경/추가 시
4. 아키텍처 변경 시
5. 사용자가 명시적으로 문서화 요청 시
...
### 기존 문서
- `docs/authentication-flow.md` - JWT 기반 인증 시스템 내부 동작 가이드
